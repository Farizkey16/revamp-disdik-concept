...
const teamMembersR1: TeamMembers[] = [
  {
    name: "Nahdiana",
    role: "Kepala Dinas",
...
const teamMembersR2: TeamMembers[] = [
  {
    name: "Taga",
...
const cultureBox: any[] = [
  {
    title: "Transparansi",
    description:
      "Mengutamakan kejujuran dan keterbukaan dalam pengelolaan pendidikan.",
    icon: <ShieldCheck className="w-8 h-8 text-blue-600" />,
  },
  {
    title: "Aksesibilitas",
    description:
      "Menjamin pendidikan inklusif dan merata untuk semua lapisan masyarakat.",
    icon: <Globe className="w-8 h-8 text-green-600" />,
  },
  {
    title: "Profesionalisme",
    description:
      "Menjaga mutu layanan pendidikan dengan etika dan kompetensi tinggi.",
    icon: <Briefcase className="w-8 h-8 text-gray-700" />,
  },
  {
    title: "Inovasi",
    description:
      "Mendorong penggunaan teknologi dan ide kreatif dalam pembelajaran.",
    icon: <Sparkles className="w-8 h-8 text-purple-600" />,
  },
  {
    title: "Kolaborasi",
    description:
      "Bekerja sama dengan berbagai pihak untuk menciptakan solusi pendidikan.",
  ...
  return (
    <main>
      <section className="relative w-full h-[60vh] md:h-[80vh]">
        ...
        <div className="relative flex flex-col justify-center items-center text-left z-10 mx-auto h-full text-white px-2 md:px-0">
          ...
          <p className="px-2 sm:px-4 max-w-xl sm:max-w-2xl md:max-w-3xl text-sm sm:text-base md:text-lg text-center">
            {" "}
            Dibentuk sebagai bagian dari struktur pemerintah daerah yang
            mengelola urusan pendidikan. Sejak berdirinya, Disdik Jakarta
            berfokus pada pemerataan akses pendidikan, peningkatan mutu sekolah,
            serta penyelarasan sistem pendidikan dengan tantangan zaman. Seiring
            waktu, mereka meluncurkan berbagai inisiatif seperti digitalisasi
            sekolah, pendidikan inklusif, dan revitalisasi SMK untuk menjawab
      ...
      <section className="relative z-10 mt-8">
        <h2 className="text-2xl sm:text-3xl md:text-3xl font-bold text-gray-800 mb-4 sm:mb-6 text-center py-2 rounded">
          Struktur Organisasi
        </h2>
        {/* Kepala dan Wakil */}
        <div className="flex flex-col sm:flex-row justify-center items-center gap-4 sm:gap-8">
          {teamMembersR1.map(({ name, role, imgsrc, altxt }, index: number) => (
            <Card
              key={index}
              className="w-60 sm:w-70 shadow-md hover:shadow-xl transition"
            >
